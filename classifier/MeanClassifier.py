...
class MeanClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y=None):
        ...
        # The len(X) would vary according to the k value in CV
...
if __name__ == "__main__":
    ...
    gs = GridSearchCV(MeanClassifier(),
                      tuned_params)

    # for some reason I have to pass y with same shape
    # otherwise gridsearch throws an error. Not sure why.
    gs.fit(X_train, y=None)
    print("Best Param:",gs.best_params_) # {'intValue': -10} # and that is what we expect :)

    print("Predicted Values", sum(gs.predict(X_test)))